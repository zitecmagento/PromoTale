...
class Zend_Tool_Framework_Client_Console_HelpSystem
{
    ...
    public function respondWithErrorMessage($errorMessage, Exception $exception = null)
    {
        ...
        $this->_response->appendContent($errorMessage, array('indention' => 1, 'blockize' => 72, 'color' => array('white',
                'bgRed')));
    ...
    public function respondWithGeneralHelp()
    {
        ...
        $this->_response->appendContent('Usage:', array('color' => 'green'))
                ...
                ->appendContent('    Note: You may use "?" in any place of the above usage string to ask for more specific help information.', array(
                    'color' => 'yellow'))
                ->appendContent('    Example: "zf ? version" will list all available actions for the version provider.', array(
                    'color' => 'yellow', 'separator' => 2))
    ...
    public function respondWithSpecialtyAndParamHelp($providerName, $actionName)
    {
        ...
        $this->_response->appendContent(
                'Details for action "' . $actionName . '" and provider "' . $providerName . '"', array('color' => 'green')
        );
        ...
        foreach ($displayProviderMetadatas as $providerMetadata) {
            ...
            foreach ($providerSignature->getActions() as $actionInfo) {
                ...
                if (!$includeAllSpecialties && count($actionableGlobalMetadatas) > 1) {
                    ...
                    $this->_response->appendContent(
                            'zf ' . $actionMetadata->getValue() . ' ' . $providerMetadata->getValue() . '.?', array('color' => 'cyan',
                        'separator' => false)
                    );
    ...
    Zend_Tool_Framework_Metadata_Tool $providerMetadata, Zend_Tool_Framework_Metadata_Tool $actionMetadata, Zend_Tool_Framework_Metadata_Tool $specialtyMetadata, Zend_Tool_Framework_Metadata_Tool $parameterLongMetadata)//,
    //Zend_Tool_Framework_Metadata_Tool $parameterShortMetadata)
    {
        $this->_response->appendContent(
                '    zf ' . $actionMetadata->getValue() . ' ' . $providerMetadata->getValue(), array('color' => 'cyan', 'separator' => false)
        );
