...
class Mage_CatalogInventory_Model_Resource_Stock_Status extends Mage_Core_Model_Resource_Db_Abstract
{
    ...
    public function getProductsType($productIds)
    {
        ...
        $select = $this->_getReadAdapter()->select()
                ->from(
                        array('e' => $this->getTable('catalog/product')), array('entity_id', 'type_id')
                )
    ...
    public function addStockStatusToSelect(Varien_Db_Select $select, Mage_Core_Model_Website $website)
    {
        ...
        $select->joinLeft(
                array('stock_status' => $this->getMainTable()), 'e.entity_id = stock_status.product_id AND stock_status.website_id=' . $websiteId, array(
    ...
    {
        $select->join(
                array('ciss' => $this->getMainTable()), "ciss.product_id = {$entityField} AND ciss.website_id = {$websiteField}", array()
        );
    ...
    public function addIsInStockFilterToCollection($collection)
    {
        ...
        $collection->getSelect()
                ->join(
                        array('stock_status_index' => $this->getMainTable()), $joinCondition, array()
                )
