...
$installer->getConnection()->addColumn(
        $installer->getTable('eav/attribute'), 'used_in_product_listing', 'tinyint(1) UNSIGNED NOT NULL DEFAULT 0'
);
$installer->getConnection()->addColumn(
        $installer->getTable('eav/attribute'), 'used_for_sort_by', 'tinyint(1) UNSIGNED NOT NULL DEFAULT 0'
);

$entityTypeId = $installer->getEntityTypeId('catalog_product');
$sqlAttributes = $installer->getConnection()->quoteInto('?', Mage::getSingleton('catalog/config')->getProductCollectionAttributes()
);
$installer->run("
...
$installer->getConnection()->addKey(
        $installer->getTable('eav/attribute'), 'IDX_USED_FOR_SORT_BY', array('entity_type_id', 'used_for_sort_by')
);
$installer->getConnection()->addKey(
        $installer->getTable('eav/attribute'), 'IDX_USED_IN_PRODUCT_LISTING', array('entity_type_id', 'used_in_product_listing')
);

// Add frontent input renderer
$installer->getConnection()->addColumn(
        $installer->getTable('eav/attribute'), 'frontend_input_renderer', 'varchar(255) DEFAULT NULL AFTER `frontend_input`'
);
...
$installer->updateAttributeGroup(
        $entityTypeId, $attributeSetId, $attributeGroupId, 'attribute_group_name', 'General Information'
);
$installer->updateAttributeGroup(
        $entityTypeId, $attributeSetId, $attributeGroupId, 'sort_order', '10'
);
