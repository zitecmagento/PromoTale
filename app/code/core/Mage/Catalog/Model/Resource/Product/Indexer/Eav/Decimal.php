...
class Mage_Catalog_Model_Resource_Product_Indexer_Eav_Decimal extends Mage_Catalog_Model_Resource_Product_Indexer_Eav_Abstract
{
    ...
    protected function _prepareIndex($entityIds = null, $attributeId = null)
    {
        ...
        $select = $write->select()
                ->from(
                        array('pdd' => $this->getValueTable('catalog/product', 'decimal')), array('entity_id', 'attribute_id'))
                ->join(
                        array('cs' => $this->getTable('core/store')), '', array('store_id'))
                ->joinLeft(
                        array('pds' => $this->getValueTable('catalog/product', 'decimal')), 'pds.entity_id = pdd.entity_id AND pds.attribute_id = pdd.attribute_id'
                        . ' AND pds.store_id=cs.store_id', array('value' => $productValueExpression))
