...
class Mage_Catalog_Model_Resource_Url extends Mage_Core_Model_Resource_Db_Abstract
{
    ...
    public function getLastUsedRewriteRequestIncrement($prefix, $suffix, $storeId)
    {
        ...
        $urlIncrementPartExpression = Mage::getResourceHelper('eav')
                ->getCastToIntExpression($adapter->getSubstringSql(
                        $requestPathField, strlen($prefix) + 1, $adapter->getLengthSql($requestPathField) . ' - ' . strlen($prefix) . ' - ' . strlen($suffix)
        ));
        $select = $adapter->select()
    ...
    protected function _getCategoryAttribute($attributeCode, $categoryIds, $storeId)
    {
        ...
        if ($this->_categoryAttributes[$attributeCode]['is_static']) {
            $select
                    ->from(
                            $this->getTable('catalog/category'), array('value' => $attributeCode, 'entity_id' => 'entity_id')
                    )
            ...
            $select
                    ->from(
                            array('t1' => $attributeTable), array('entity_id', 'value' => $valueExpr)
                    )
                    ->joinLeft(
                            array('t2' => $attributeTable), 't1.entity_id = t2.entity_id AND t1.attribute_id = t2.attribute_id AND t2.store_id = :store_id', array()
                    )
                    ->where('t1.store_id = ?', 0)
    ...
    protected function _getCategories($categoryIds, $storeId = null, $path = null)
    {
        ...
        $select->joinLeft(array('d' => $table), 'd.attribute_id = :attribute_id AND d.store_id = 0 AND d.entity_id = main_table.entity_id', array()
                )
                ->joinLeft(array('c' => $table), 'c.attribute_id = :attribute_id AND c.store_id = :store_id AND c.entity_id = main_table.entity_id', array()
        );
    ...
    protected function _getProducts($productIds, $storeId, $entityId, &$lastEntityId)
    {
        ...
        $select = $adapter->select()
                ...
                ->join(
                        array('w' => $this->getTable('catalog/product_website')), 'e.entity_id = w.product_id AND w.website_id = :website_id', array()
                )
        ...
        if ($products) {
            $select = $adapter->select()
                    ->from(
                            $this->getTable('catalog/category_product'), array('product_id', 'category_id')
                    )
    ...
    public function clearCategoryProduct($storeId)
    {
        ...
        $select = $adapter->select()
                ...
                ->joinLeft(
                        array('tcp' => $this->getTable('catalog/category_product')), 'tur.category_id = tcp.category_id AND tur.product_id = tcp.product_id', array()
                )
    ...
    public function clearStoreProductsInvalidRewrites($storeId, $productId = null)
    {
        ...
        $select = $adapter->select()
                ...
                ->joinLeft(
                        array('website' => $this->getTable('catalog/product_website')), 'rewrite.product_id = website.product_id AND website.website_id = :website_id', array()
    ...
    public function getRewriteByProductStore(array $products)
    {
        ...
        $select = $adapter->select()
                ->from(
                        array('i' => $this->getTable('catalog/category_product_index')), array('product_id', 'store_id',
                    'visibility')
                )
                ->joinLeft(
                array('r' => $this->getMainTable()), 'i.product_id = r.product_id AND i.store_id=r.store_id AND r.category_id IS NULL', array(
            'request_path')
        );
