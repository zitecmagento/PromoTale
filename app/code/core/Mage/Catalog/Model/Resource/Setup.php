...
class Mage_Catalog_Model_Resource_Setup extends Mage_Eav_Model_Entity_Setup
{
        ...
        if (is_array($categories)) {
            foreach ($categories as $category) {
                ...
                $this
                        ->getConnection()
                        ->update(
                                $this->getTable('catalog/category'), array('path' => $path), array('entity_id = ?' => $category['entity_id'])
                );
            }
        }
        return $this;
    ...
    public function rebuildCategoryLevels()
    {
        ...
        foreach ($categories as $category) {
            ...
            $adapter->update(
                    $this->getTable('catalog/category'), array('level' => $level), array('entity_id = ?' => $category['entity_id'])
            );
        }
        return $this;
    }
