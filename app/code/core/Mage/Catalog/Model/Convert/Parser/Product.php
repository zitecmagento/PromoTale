...
class Mage_Catalog_Model_Convert_Parser_Product extends Mage_Eav_Model_Convert_Parser_Abstract
{
    ...
    public function getStore()
    {
        if (is_null($this->_store)) {
            ...
            {
                $this->addException(
                        Mage::helper('catalog')->__('Invalid store specified'), Varien_Convert_Exception::FATAL
                );
                throw $e;
            }
            $this->_store = $store;
    ...
    public function parse()
    {
        ...
        foreach ($data as $i => $row) {
            ...
            try
            {
                ...
                if (empty($row['sku'])) {
                    $this->addException(
                            Mage::helper('catalog')->__('Missing SKU, skipping the record.'), Mage_Dataflow_Model_Convert_Exception::ERROR
                    );
                    continue;
                }
                ...
                if (!$row['attribute_set_id']) {
                    $this->addException(
                            Mage::helper('catalog')->__('Invalid attribute set specified, skipping the record.'), Mage_Dataflow_Model_Convert_Exception::ERROR
                    );
                    continue;
                }
                ...
                if (!$row['type_id']) {
                    $this->addException(
                            Mage::helper('catalog')->__('Invalid product type specified, skipping the record.'), Mage_Dataflow_Model_Convert_Exception::ERROR
                    );
                    continue;
                }
                ...
                if (!$storeIds) {
                    $this->addException(
                            Mage::helper('catalog')->__('Invalid store specified, skipping the record.'), Mage_Dataflow_Model_Convert_Exception::ERROR
                    );
                    continue;
                }
...
//                            );
                        ...
                        if ($attribute->usesSource()) {
                            ...
                            if (is_null($optionId)) {
                                $rowError = true;
                                $this->addException(
                                        Mage::helper('catalog')->__('Invalid attribute option specified for attribute %s (%s), skipping the record.', $field, $value), Mage_Dataflow_Model_Convert_Exception::ERROR
                                );
                                continue;
                            }
                            $value = $optionId;
            ...
            catch (Exception $e)
            {
                if (!$e instanceof Mage_Dataflow_Model_Convert_Exception) {
                    $this->addException(
                            Mage::helper('catalog')->__('Error during retrieval of option value: %s', $e->getMessage()), Mage_Dataflow_Model_Convert_Exception::FATAL
                    );
                }
            }
        }
            ...
            foreach ($product->getData() as $field => $value) {
                ...
                if ($attribute->usesSource()) {
                    ...
                    if ($value && empty($option) && $option != '0') {
                        $this->addException(
                                Mage::helper('catalog')->__('Invalid option ID specified for %s (%s), skipping the record.', $field, $value), Mage_Dataflow_Model_Convert_Exception::ERROR
                        );
                        continue;
                    }
                    if (is_array($option)) {
