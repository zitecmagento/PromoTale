    ...
    protected function _drawPackageBlock(Zend_Pdf_Page $page)
    {
        ...
        foreach ($packages as $packageId => $package) {
            ...
            if ($packaging->displayCustomsValue()) {
                $page->drawText(
                        Mage::helper('sales')->__('Customs Value'), $itemCollsX[++$i] + $txtIndent, $this->y, 'UTF-8'
                );
            }
            $page->drawText(
            ...
            foreach ($package->getItems() as $itemId => $item) {
                ...
                if ($packaging->displayCustomsValue()) {
                    $page->drawText(
                            $packaging->displayPrice($item->getCustomsValue()), $itemCollsX[++$i] + $txtIndent, $this->y, 'UTF-8'
                    );
                }
                $page->drawText(
                        $packaging->getQtyOrderedItem($item->getOrderItemId()), $itemCollsX[++$i] + $txtIndent, $this->y, 'UTF-8'
                );
