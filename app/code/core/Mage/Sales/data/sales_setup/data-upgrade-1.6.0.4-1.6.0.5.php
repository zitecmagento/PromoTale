...
$subSelect = $installer->getConnection()->select()
        ->from(
                array('citem' => $installer->getTable('sales/creditmemo_item')), array(
        ...
        ->joinLeft(
                array('c' => $installer->getTable('sales/creditmemo')), 'c.entity_id = citem.parent_id', array()
        )
        ->joinLeft(
                array('o' => $installer->getTable('sales/order')), 'o.entity_id = c.order_id', array()
        )
        ->joinLeft(
                array('oitem' => $installer->getTable('sales/order_item')), 'oitem.order_id = o.entity_id AND oitem.product_id=citem.product_id', array(
            'item_id')
        )
        ->group('oitem.item_id');
...
$select = $installer->getConnection()->select()
        ->from(
                array('selected' => $subSelect), array(
...
$updateQuery = $installer->getConnection()->updateFromSelect(
        $select, array('main' => $installer->getTable('sales/order_item'))
);
