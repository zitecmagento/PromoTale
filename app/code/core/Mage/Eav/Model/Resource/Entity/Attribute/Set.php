    ...
    public function getSetInfo(array $attributeIds, $setId = null)
    {
        ...
        if (count($attributeIds) > 0) {
            $select = $adapter->select()
                    ...
                    ->joinLeft(
                            array('attribute_group' => $this->getTable('eav/attribute_group')), 'entity.attribute_group_id = attribute_group.attribute_group_id', array(
        ...
        foreach ($attributeIds as $atttibuteId) {
            $setInfo[$atttibuteId] = isset($attributeToSetInfo[$atttibuteId]) ? $attributeToSetInfo[$atttibuteId] : array();
        }
