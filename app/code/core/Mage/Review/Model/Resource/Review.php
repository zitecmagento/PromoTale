...
class Mage_Review_Model_Resource_Review extends Mage_Core_Model_Resource_Db_Abstract
{
    ...
    protected function _getLoadSelect($field, $value, $object)
    {
        ...
        $select->join(
                $this->_reviewDetailTable, $this->getMainTable() . ".review_id = {$this->_reviewDetailTable}.review_id"
        );
        return $select;
    }
    ...
    public function afterDeleteCommit(Mage_Core_Model_Abstract $object)
    {
        ...
        $this->_aggregateRatings(
                $this->_deleteCache['ratingIds'], $this->_deleteCache['entityPkValue']
        );
        $this->_deleteCache = array();
    ...
    public function getTotalReviews($entityPkValue, $approvedOnly = false, $storeId = 0)
    {
        ...
        if ($storeId > 0) {
            $select->join(array('store' => $this->_reviewStoreTable), $this->_reviewTable . '.review_id=store.review_id AND store.store_id = :store_id', array());
    ...
    public function aggregate($object)
    {
        ...
        foreach ($ratingSummaries as $ratingSummaryObject) {
            ...
            $reviewsCount = $this->getTotalReviews(
                    $object->getEntityPkValue(), true, $ratingSummaryObject->getStoreId()
            );
