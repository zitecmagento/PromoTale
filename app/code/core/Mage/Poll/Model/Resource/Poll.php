...
class Mage_Poll_Model_Resource_Poll extends Mage_Core_Model_Resource_Db_Abstract
{
    ...
    protected function _getSelectIds($object)
    {
        ...
        if ($storeId) {
            $select->join(
                    array('store' => $this->getTable('poll/poll_store')), 'main_table.poll_id=store.poll_id AND store.store_id = ' . $read->quote($storeId), array()
            );
        }
    ...
    public function resetVotesCount($object)
    {
        ...
        $adapter->update(
                $this->getMainTable(), array('votes_count' => new Zend_Db_Expr("($select)")), array('poll_id = ' . $adapter->quote($object->getPollId()))
        );
        return $object;
    }
    ...
    public function lookupStoreIds($id)
    {
        return $this->_getReadAdapter()->fetchCol(
                        $this->_getReadAdapter()->select()
                                ...
                                ->where("{$this->getIdFieldName()} = :id_field"), array(':id_field' => $id)
        );
    }
