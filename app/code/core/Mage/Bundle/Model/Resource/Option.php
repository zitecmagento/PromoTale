...
class Mage_Bundle_Model_Resource_Option extends Mage_Core_Model_Resource_Db_Abstract
{
    ...
    protected function _afterDelete(Mage_Core_Model_Abstract $object)
    {
        ...
        $this->_getWriteAdapter()->delete(
                $this->getTable('bundle/option_value'), array('option_id = ?' => $object->getId())
        );
    ...
    public function getSearchableData($productId, $storeId)
    {
        ...
        $title = $adapter->getCheckSql('option_title_store.title IS NOT NULL', 'option_title_store.title', 'option_title_default.title'
        );
        $bind = array(
            'store_id' => $storeId,
        ...
        $select = $adapter->select()
                ...
                ->join(
                        array('option_title_default' => $this->getTable('bundle/option_value')), 'option_title_default.option_id = opt.option_id AND option_title_default.store_id = 0', array()
                )
                ->joinLeft(
                        array('option_title_store' => $this->getTable('bundle/option_value')), 'option_title_store.option_id = opt.option_id AND option_title_store.store_id = :store_id', array(
                    'title' => $title)
                )
