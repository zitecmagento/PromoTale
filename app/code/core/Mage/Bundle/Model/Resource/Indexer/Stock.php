...
class Mage_Bundle_Model_Resource_Indexer_Stock extends Mage_CatalogInventory_Model_Resource_Indexer_Stock_Default
{
    ...
    protected function _prepareBundleOptionStockData($entityIds = null, $usePrimaryTable = false)
    {
        ...
        $select->columns('website_id', 'cw')
                ...
                ->joinLeft(
                        array('bs' => $this->getTable('bundle/selection')), 'bs.option_id = bo.option_id', array()
                )
                ->joinLeft(
                        array('i' => $idxTable), 'i.product_id = bs.product_id AND i.website_id = cw.website_id AND i.stock_id = cis.stock_id', array()
                )
                ->joinLeft(
                        array('e' => $this->getTable('catalog/product')), 'e.entity_id = bs.product_id', array()
                )
    ...
    protected function _getStockStatusSelect($entityIds = null, $usePrimaryTable = false)
    {
        ...
        $select->columns('cw.website_id')
                ...
                ->joinLeft(
                        array('o' => $this->_getBundleOptionTable()), 'o.entity_id = e.entity_id AND o.website_id = cw.website_id AND o.stock_id = cis.stock_id', array()
                )
        ...
        if ($this->_isManageStock()) {
            $statusExpr = $adapter->getCheckSql(
                    'cisi.use_config_manage_stock = 0 AND cisi.manage_stock = 0', '1', 'cisi.is_in_stock'
            );
        } else {
            $statusExpr = $adapter->getCheckSql(
                    'cisi.use_config_manage_stock = 0 AND cisi.manage_stock = 1', 'cisi.is_in_stock', '1'
            );
        }
