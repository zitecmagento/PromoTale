    ...
    public function aggregate($tagIds = null)
    {
        ...
        try
        {
            ...
            $select = $writeAdapter->select()
                    ->from(
                            array('tr' => $this->getTable('tag/relation')), array(
                        'tr.tag_id',
                        'tr.store_id',
                        ...
                        . $writeAdapter->getCheckSql(
                                'tp.base_popularity IS NOT NULL', 'tp.base_popularity', '0'
                        )
                        . ')',
                    ...
                    ->joinInner(
                            array('cs' => $this->getTable('core/store')), 'cs.store_id = tr.store_id', array()
                    )
                    ->joinInner(
                            array('pw' => $this->getTable('catalog/product_website')), 'cs.website_id = pw.website_id AND tr.product_id = pw.product_id', array()
                    )
                    ->joinInner(
                            array('e' => $this->getTable('catalog/product')), 'tr.product_id = e.entity_id', array()
                    )
                    ->joinLeft(
                            array('tp' => $this->getTable('tag/properties')), 'tp.tag_id = tr.tag_id AND tp.store_id = tr.store_id', array()
                    )
                    ->group(array(
                        'tr.tag_id',
                        'tr.store_id'
