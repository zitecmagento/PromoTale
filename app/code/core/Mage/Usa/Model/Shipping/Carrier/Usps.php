    ...
    public function setRequest(Mage_Shipping_Model_Rate_Request $request)
    {
        ...
        if ($request->getOrigPostcode()) {
            ...
            $r->setOrigPostal(Mage::getStoreConfig(
                            Mage_Shipping_Model_Shipping::XML_PATH_STORE_ZIP, $request->getStoreId()
            ));
        }
            ...
            if (strpos(trim($response), '<?xml') === 0) {
                if (strpos($response, '<?xml version="1.0"?>') !== false) {
                    $response = str_replace(
                            '<?xml version="1.0"?>', '<?xml version="1.0" encoding="ISO-8859-1"?>', $response
                    );
                }
                            ...
                            foreach ($xml->Package->Postage as $postage) {
                                ...
                                if (in_array($serviceCode, $allowedMethods)) {
                                    ...
                                    $priceArr[$serviceCode] = $this->getMethodPrice(
                                            (string) $postage->Rate, $serviceCode
                                    );
                                }
                            }
                            asort($priceArr);
                        ...
                        if (is_object($xml->Package) && is_object($xml->Package->Service)) {
                            foreach ($xml->Package->Service as $service) {
                                ...
                                if (in_array($serviceCode, $allowedMethods)) {
                                    ...
                                    $priceArr[$serviceCode] = $this->getMethodPrice(
                                            (string) $service->Postage, $serviceCode);
                                }
                            }
                            asort($priceArr);
                        }
                ...
                if (empty($priceArr)) {
                    ...
                    foreach ($priceArr as $method => $price) {
                        ...
                        $rate->setMethodTitle(
                                isset($serviceCodeToActualNameMap[$method]) ? $serviceCodeToActualNameMap[$method] : $this->getCode('method', $method)
                        );
    ...
    protected function _formUsExpressShipmentRequest(Varien_Object $request)
    {
        ...
        if ($packageParams->getWeightUnits() != Zend_Measure_Weight::OUNCE) {
            $packageWeight = round(Mage::helper('usa')->convertMeasureWeight(
                            $request->getPackageWeight(), $packageParams->getWeightUnits(), Zend_Measure_Weight::OUNCE
            ));
        }
    ...
    protected function _formIntlShipmentRequest(Varien_Object $request)
    {
        ...
        if ($packageParams->getGirthDimensionUnits() != Zend_Measure_Length::INCH) {
            $girth = round(Mage::helper('usa')->convertMeasureDimension(
                            $packageParams->getGirth(), $packageParams->getGirthDimensionUnits(), Zend_Measure_Length::INCH
            ));
        }
        ...
        foreach ($packageItems as $itemShipment) {
            ...
            if ($packageParams->getWeightUnits() != Zend_Measure_Weight::POUND) {
                $itemWeight = Mage::helper('usa')->convertMeasureWeight(
                        $itemWeight, $packageParams->getWeightUnits(), Zend_Measure_Weight::POUND
                );
            }
