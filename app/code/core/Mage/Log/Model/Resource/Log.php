...
class Mage_Log_Model_Resource_Log extends Mage_Core_Model_Resource_Db_Abstract
{
    ...
    protected function _cleanVisitors($time)
    {
        ...
        while (true) {
            $select = $readAdapter->select()
                    ->from(
                            array('visitor_table' => $this->getTable('log/visitor')), array('visitor_id' => 'visitor_table.visitor_id'))
                    ->joinLeft(
                            array('customer_table' => $this->getTable('log/customer')), 'visitor_table.visitor_id = customer_table.visitor_id AND customer_table.log_id IS NULL', array())
    ...
    protected function _cleanCustomers($time)
    {
        ...
        $select = $readAdapter->select()
                ->from(
                        array('log_customer_main' => $this->getTable('log/customer')), array('log_id'))
                ->joinLeft(
                        array('log_customer' => $this->getTable('log/customer')), 'log_customer_main.customer_id = log_customer.customer_id '
                        . 'AND log_customer_main.log_id < log_customer.log_id', array())
        ...
        while (true) {
            ...
            $select = $readAdapter->select()
                    ->from(
                            $this->getTable('log/customer'), array('log_id', 'visitor_id'))
    ...
    protected function _cleanUrls()
    {
        ...
        while (true) {
            $select = $readAdapter->select()
                    ->from(
                            array('url_info_table' => $this->getTable('log/url_info_table')), array('url_id'))
                    ->joinLeft(
                            array('url_table' => $this->getTable('log/url_table')), 'url_info_table.url_id = url_table.url_id', array())
            ...
            $writeAdapter->delete(
                    $this->getTable('log/url_info_table'), array('url_id IN (?)' => $urlIds)
            );
        }
