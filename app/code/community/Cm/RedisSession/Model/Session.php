...
class Cm_RedisSession_Model_Session extends Mage_Core_Model_Mysql4_Session
{
    ...
    public function __construct()
    {
        ...
        if ($this->_logLevel >= 7) {
            Mage::log(
                    sprintf(
                            "%s: %s initialized for connection to %s:%s after %.5f seconds", $this->_getPid(), get_class($this), $host, $port, (microtime(true) - $this->_timeStart)
                    ), Zend_Log::DEBUG, self::LOG_FILE
            );
            if ($this->_isBot) {
                Mage::log(
                        sprintf(
                                "%s: Bot detected for user agent: %s", $this->_getPid(), $userAgent
                        ), Zend_Log::DEBUG, self::LOG_FILE
                );
            }
        }
    ...
    public function hasConnection()
    {
        ...
        try
        {
            ...
            if ($this->_logLevel >= 7) {
                Mage::log(
                        sprintf("%s: Connected to Redis", $this->_getPid()
                        ), Zend_Log::DEBUG, self::LOG_FILE
                );
                // reset timer
                $this->_timeStart = microtime(true);
        ...
        catch (Exception $e)
        {
            ...
            if ($this->_logLevel >= 0) {
                Mage::log(
                        sprintf(
                                "%s: Unable to connect to Redis; falling back to MySQL handler", $this->_getPid()
    ...
    public function read($sessionId)
    {
        ...
        if ($this->_logLevel >= 7) {
            Mage::log(
                    sprintf(
                            "%s: Attempting read lock on ID %s", $this->_getPid(), $sessionId
                    ), Zend_Log::DEBUG, self::LOG_FILE
            );
            // reset timer
            $this->_timeStart = microtime(true);
        ...
        while (1) {
            ...
            if ($lock == 1 || ($tries >= $this->_breakAfter && $lock % self::BREAK_MODULO == 0)) {
                ...
                if ($this->_logLevel >= 6) {
                    ...
                    if ($this->_logLevel >= 7) {
                        $additionalDetails = sprintf(
                                        "after %.5f seconds ", (microtime(true) - $this->_timeStart), $tries
                                ) . $additionalDetails;
                    }
                    ...
                    if ($lock != 1) {
                        Mage::log(
                                sprintf("%s: Successfully broke lock for ID %s %s. Lock: %s, BREAK_MODULO: %s\nLast request of broken lock: %s", $this->_getPid(), $sessionId, $additionalDetails, $lock, self::BREAK_MODULO, $this->_redis->hGet($sessionId, 'req')
            ...
            // Otherwise, add to "wait" counter and continue
                ...
                do {
                    ...
                    if ($this->_logLevel >= 7) {
                        Mage::log(
                                sprintf(
                                        "%s: Waiting for lock on ID %s (%s tries, %s waiting, %.5f seconds elapsed)", $this->_getPid(), $sessionId, $tries, $waiting, (microtime(true) - $this->_timeStart)
                                ), Zend_Log::DEBUG, self::LOG_FILE
                        );
                    }
            ...
            // Handle overloaded sessions
                ...
                if ($detectZombies) {
                    ...
                    if ($lock > $oldLock                 // lock shouldn't be less than old lock (another process broke the lock)
                        ...
                        if ($this->_logLevel >= 6) {
                            Mage::log(
                                    sprintf("%s: Detected zombie waiter after %.5f seconds for ID %s (%s waiting)\n  %s (%s - %s)", $this->_getPid(), (microtime(true) - $this->_timeStart), $sessionId, $waiting, Mage::app()->getRequest()->getRequestUri(), Mage::app()->getRequest()->getClientIp(), Mage::app()->getRequest()->getHeader('User-Agent')
                                    ), Zend_Log::INFO, self::LOG_FILE
                            );
                        }
                        $waiting = $this->_redis->hIncrBy($sessionId, 'wait', -1);
                ...
                if ($waiting >= $this->_maxConcurrency) {
                    ...
                    if ($this->_logLevel >= 4) {
                        Mage::log(
                                sprintf("%s: Session concurrency exceeded for ID %s; displaying HTTP 503 (%s waiting, %s total requests)\n  %s (%s - %s)", $this->_getPid(), $sessionId, $waiting, $writes, Mage::app()->getRequest()->getRequestUri(), Mage::app()->getRequest()->getClientIp(), Mage::app()->getRequest()->getHeader('User-Agent')
            ...
            if ($tries % self::DETECT_ZOMBIES == 0) {
                if ($this->_logLevel >= 7) {
                    Mage::log(
                            sprintf(
                                    "%s: Checking for zombies after %.5f seconds of waiting...", $this->_getPid(), (microtime(true) - $this->_timeStart)
                            ), Zend_Log::DEBUG, self::LOG_FILE
                    );
                }
                ...
                if ($pid && !$this->_pidExists($pid)) {
                    ...
                    if ($this->_logLevel >= 6) {
                        Mage::log(
                                sprintf("%s: Detected zombie process (%s) for %s (%s waiting)\n  %s (%s - %s)", $this->_getPid(), $pid, $sessionId, $waiting, Mage::app()->getRequest()->getRequestUri(), Mage::app()->getRequest()->getClientIp(), Mage::app()->getRequest()->getHeader('User-Agent')
                                ), Zend_Log::INFO, self::LOG_FILE
                        );
                    }
                    continue;
            ...
            if ($tries >= $this->_breakAfter + self::FAIL_AFTER) {
                ...
                if ($this->_logLevel >= 5) {
                    ...
                    Mage::log(
                            sprintf(
                                    "%s: Giving up on read lock for ID %s %s", $this->_getPid(), $sessionId, $additionalDetails
        ...
        if ($this->_logLevel >= 7) {
            Mage::log(
                    sprintf(
                            "%s: Data read for ID %s after %.5f seconds", $this->_getPid(), $sessionId, (microtime(true) - $this->_timeStart)
                    ), Zend_Log::DEBUG, self::LOG_FILE
            );
        }
    ...
    public function write($sessionId, $sessionData)
    {
        ...
        if ($this->_sessionWritten) {
            if ($this->_logLevel >= 7) {
                Mage::log(
                        sprintf(
                                "%s: Repeated session write detected; skipping for ID %s", $this->_getPid(), $sessionId
                        ), Zend_Log::DEBUG, self::LOG_FILE
                );
            }
            return TRUE;
        ...
        if ($this->_logLevel >= 7) {
            Mage::log(
                    sprintf(
                            "%s: Attempting write to ID %s", $this->_getPid(), $sessionId
                    ), Zend_Log::DEBUG, self::LOG_FILE
            );
            // reset timer
            $this->_timeStart = microtime(true);
        ...
        {
            if ($this->_dbNum)
                $this->_redis->select($this->_dbNum);  // Prevent conflicts with other connections?
            $pid = $this->_redis->hGet('sess_' . $sessionId, 'pid'); // PHP Fatal errors cause self::SESSION_PREFIX to not work..
            if (!$pid || $pid == $this->_getPid()) {
                if ($this->_logLevel >= 7) {
                    Mage::log(
                            sprintf(
                                    "%s: Write lock obtained on ID %s", $this->_getPid(), $sessionId
                            ), Zend_Log::DEBUG, self::LOG_FILE
                    );
                }
                ...
                if ($this->_logLevel >= 7) {
                    Mage::log(
                            sprintf(
                                    "%s: Data written to ID %s after %.5f seconds", $this->_getPid(), $sessionId, (microtime(true) - $this->_timeStart)
                            ), Zend_Log::DEBUG, self::LOG_FILE
                    );
                }
            } else {
                if ($this->_logLevel >= 4) {
                    if ($this->_hasLock) {
                        Mage::log(
                                sprintf("%s: Unable to write session after %.5f seconds, another process took the lock for ID %s", $this->_getPid(), (microtime(true) - $this->_timeStart), $sessionId
                                ), Zend_Log::WARN, self::LOG_FILE
                        );
                    } else {
                        Mage::log(
                                sprintf("%s: Unable to write session after %.5f seconds, unable to acquire lock on ID %s", $this->_getPid(), (microtime(true) - $this->_timeStart), $sessionId
                                ), Zend_Log::WARN, self::LOG_FILE
                        );
                    }
                }
    ...
    public function destroy($sessionId)
    {
        ...
        if ($this->_logLevel >= 7) {
            Mage::log(
                    sprintf(
                            "%s: Destroying ID %s", $this->_getPid(), $sessionId
                    ), Zend_Log::DEBUG, self::LOG_FILE
            );
        }
        $this->_redis->pipeline();
    ...
    public function close()
    {
        ...
        if ($this->_logLevel >= 7) {
            Mage::log(
                    sprintf(
                            "%s: Closing connection", $this->_getPid()
                    ), Zend_Log::DEBUG, self::LOG_FILE
            );
        }
        if ($this->_redis)
    ...
    public function _encodeData($data)
    {
        ...
        if ($this->_compressionThreshold > 0 && $this->_compressionLib != 'none' && $originalDataSize >= $this->_compressionThreshold) {
            if ($this->_logLevel >= 7) {
                Mage::log(
                        sprintf(
                                "%s: Compressing %s bytes with %s", $this->_getPid(), $originalDataSize, $this->_compressionLib
                        ), Zend_Log::DEBUG, self::LOG_FILE
                );
                // reset timer
                $this->_timeStart = microtime(true);
            ...
            if ($data) {
                ...
                if ($this->_logLevel >= 7) {
                    Mage::log(
                            sprintf(
                                    "%s: Data compressed by %.1f percent in %.5f seconds", $this->_getPid(), ($originalDataSize == 0 ? 0 : (100 - (strlen($data) / $originalDataSize * 100))), (microtime(true) - $this->_timeStart)
                            ), Zend_Log::DEBUG, self::LOG_FILE
                    );
                }
            } else if ($this->_logLevel >= 4) {
                Mage::log(
                        sprintf("%s: Could not compress session data using %s", $this->_getPid(), $this->_compressionLib
                        ), Zend_Log::WARN, self::LOG_FILE
                );
            }
        }
